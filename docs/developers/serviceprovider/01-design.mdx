...
sequenceDiagram
    ...
    sp->>ds: deploys
    sp-->>usr: installs DomainServiceAPI in user ControlPlane
...
Main tasks towards ControlPlane/Workload Clusters (based on watching the `ServiceProviderAPI`):
...
In this context, `reconcileScope` holds the `ServiceProviderConfig` and provides clients to access onboarding, ControlPlane and workload clusters.

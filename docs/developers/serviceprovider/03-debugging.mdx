...
| **Onboarding** | ControlPlane and ServiceProviderAPI objects |
| **ControlPlane** | DomainService CRDs and deployed resources |
...
This is especially useful when you need to cross-reference resources across the platform, ControlPlane, and workload clusters during a debugging session. See the [kw repo](https://github.com/Diaphteiros/kw) for the full list of commands and features.
...
The `DEV_DEBUG=true` flag activates local debug mode: instead of using the in-cluster API server addresses for the Onboarding, ControlPlane or Workload clusters, the controller reads the `kind.clusters.openmcp.cloud/localhost` annotation from the corresponding `AccessRequest` objects and uses those locally-reachable addresses instead.
:::tip
...
If your controller fails to connect to the ControlPlane or workload cluster, verify that the AccessRequest was created and that the corresponding kubeconfig secret exists:

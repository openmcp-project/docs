...
The service provider honours the [operation annotations](https://open-control-plane.io/developers/general#operation-annotations) defined in the controller guidelines. These annotations allow platform owners to pause, force-reconcile, or otherwise control the behaviour of a managed resource without modifying its spec.

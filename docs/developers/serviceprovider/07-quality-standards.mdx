...
The service provider honours the [operation annotations](https://open-control-plane.io/developers/general#operation-annotations) defined in the controller guidelines. These annotations allow platform owners to pause or force-reconcile a `ServiceProviderAPI` without modifying its spec.

...
The service provider honours the [operation annotations](/developers/general#operation-annotations) defined in the controller guidelines. These annotations allow platform owners to pause or force-reconcile a `ServiceProviderAPI` or `ServiceProviderConfig` without modifying its spec.

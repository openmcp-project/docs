...
Replace api, name and module with something fitting your service:
...
Optional flags:
- `workloadcluster` enables workload cluster support (see below) 
- `secretwatcher` generates a [secret watcher](#watch-secrets-for-changes) implementation
- `samplecode` generates sample code to get started quickly
...
The template contains a Taskfile_template.yaml which can be removed after the template was rendered. If you want to 

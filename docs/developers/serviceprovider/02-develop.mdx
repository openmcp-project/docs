...
- **An optional workload cluster**, provisioned when using the template flag `-w`. Your service provider then requests a workload cluster from the `openmcp-operator` to deploy its workload outside the ControlPlane. This will result in another kind cluster.
...
```go
mcpTokenAccess := &clustersv1alpha1.TokenConfig{
...
Both generators will be called during reconciliation. The `AdditionalDataGenerators` will generate additional data which is looped through the various `ClusterAccessReconciler` methods down to the `TokenAccessGenerator` where it can be used to tailor the `TokenConfig` to the permission requirements of your service provider.
This is for example useful if your service provider offers a namespace override. The namespace is only known during reconciliation and using a `TokenAccessGenerator` allows you to take that information into account.
...
To see a real-world example on how to implement dynamic cluster access please check out the implementation of [service-provider-external-secrets](https://github.com/openmcp-project/service-provider-external-secrets) TODO add correct link.
...
To achieve this we first need to create a named type which will hold the namespace and function which determines it.
...
func resolveFooServiceNamespace(_ context.Context, obj *fooservicesv1alpha1.FooService, providerConfig *fooservicesv1alpha1.ProviderConfig) (any, error) {
	namespace := fooService.DefaultNamespace
	...
	if obj != nil && providerConfig != nil {
		for _, version := range providerConfig.Spec.Versions {
			...
			helmValues, err := fooservice.ExtractHelmValues(version.HelmValues)
			if err != nil {
...
Now we define a `TokenAccessGenerator` which tries to fetch the namespace from the provided additionalData and then uses that to set the `Namespace` in the `Permissions` of our `TokenConfig`.

```go
tokenAccessGenerator := func(req reconcile.Request, additionalData ...any) (*clustersv1alpha1.TokenConfig, error) {
    namespace := fooService.DefaultNamespace
...
Next we need to register the `TokenAccessGenerator` with our `ClusterRequest`...
...
... and our `ClusterRequest` with our `ClusterAccessReconciler`.
...
To tie it all together we have to register `resolveFooServiceNamespace` as `AdditionalDataGenerator` with our `APIReconciler` along with our `ClusterAccessReconciler`.
...
Then we run `go mod tidy` afterwards to update our dependencies.

#### 2. Add imports

Next we need to add the `advanced` sub-package and the controller-runtime `reconcile` package to our `main.go` imports:
...
If you need the permissions to vary per reconcile request please refer to [Dynamic Cluster Access](#dynamic-cluster-access).
:::

#### 4. Build the `ClusterRequest`

Next we need to create an `ClusterRequest` for our MCP cluster where we register our `TokenConfig`
...
```go
car := advanced.NewClusterAccessReconciler(platformCluster.Client(), "YourOperatorName").
    WithManagedLabels(func(controllerName string, req reconcile.Request, reg advanced.ClusterRegistration) (string, string, map[string]string) {
        _, managedPurpose, _ := advanced.DefaultManagedLabelGenerator(controllerName, req, reg)
        return controllerName, managedPurpose, map[string]string{
            openmcpconst.OnboardingNameLabel:      req.Name,
...
Lastly we have to register our advanced ClusterAccessReconciler with our APIReconciler. To do that we replace the `NewAPIReconcilerBuilder.ClusterAccessReconciler` with `NewAPIReconcilerBuilder.AdvancedClusterAccessReconciler` in the builder chain and are ready to go.

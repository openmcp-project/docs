...
- **A user-facing ServiceProviderAPI**: This allows end users to request a `DomainService` for a `ControlPlane`, e.g. `FooService` or `Velero`.
...
:::info
In the previous table, `r` refers to the receiver of the [CreateOrUpdate](#createorupdate-operation) and [Delete](#delete-operation) methods, while `clusters` refers to the parameter of type `ClusterContext`.
:::
...
The template allows you to create a service provider without requiring deep knowledge of the underlying OpenControlPlane platform. Run the following commands to generate a new provider.

Initiallize the [build](https://github.com/openmcp-project/build) submodule:

```bash
git submodule update --init --recursive
```

Replace api, name and module with names fitting to your service:

```bash
task template:generate-provider api=Velero name=velero module=github.com/openmcp-project/service-provider-velero samplecode=true
```

Optional flags:
- `workloadcluster=true` enables workload cluster support (see below) 
- `secretwatcher=true` generates a [secret watcher](#watch-secrets-for-changes) implementation
- `samplecode=true` generates sample code to get started quickly
- `webhook=true` generates [admission webhooks](#admission-webhooks) for your API type
...
To run the the generated end-to-end test using [task](https://taskfile.dev/)
...
- **A ControlPlane cluster** where your service provider installs its [DomainServiceAPI](https://openmcp-project.github.io/docs/about/design/service-provider#api) and optionally its workload.
- **An optional workload cluster**, provisioned when using the template flag `workloadcluster=true`. Your service provider then requests a workload cluster from the `openmcp-operator` to deploy its workload outside the ControlPlane. This will result in another kind cluster.

:::info
This means running the above command from the [Service Provider Template Usage](#service-provider-template-usage) section with `workloadcluster=true` like this:
```bash
task template:generate-provider api=Velero name=velero module=github.com/openmcp-project/service-provider-velero samplecode=true workloadcluster=true
```
:::
...
If you want to revert your changes and start fresh, simply use git and delete any generated untracked files. 

:::info
Rendering the template moves the contents of cmd/service-provider-template into a new directory cmd/service-provider-&lt;name&gt;. This directory needs to be removed manually before rendering the template again as reverting changes via git does not remove the stale directory.
:::

:::info
`Taskfile_template.yaml` is only needed for template rendering and can be safely deleted afterwards.
:::
...
3. Runtime behavior: A `ProviderConfig` **may** expose configuration settings that influence the runtime behavior of a service provider, such as the poll interval used in the service provider template or [support for custom CA bundles](./how-to-guides/01-custom-ca.md).
...
Pass the `secretwatcher=true` flag to the template generator to scaffold the secret watcher:

```bash
task template:generate-provider api=Velero name=velero module=github.com/openmcp-project/service-provider-velero samplecode=true secretwatcher=true
```
...
:::info
[Cluster and Namespace Context](#cluster-and-namespace-context) provides an overview of available clusters, their purpose and how to access them.
:::

In addition, `ClusterContext` exposes two `ObjectKeys` that can be used to retrieve the kubeconfig for either the ControlPlane or the workload cluster. These kubeconfigs can be passed to a [HelmReleases spec](https://fluxcd.io/flux/components/helm/api/v2/#helm.toolkit.fluxcd.io/v2.HelmReleaseSpec) when a service provider uses Flux to deploy its service.
...
When a user removes a service, the provider must delete all managed resources. A managed resource is any resource the service provider explicitly creates in its [CreateOrUpdate Operation](#createorupdate-operation).
For example, the `HelmRelease` in [Flux based](#for-complex-deployments-with-lot-of-moving-parts) service providers is considered a managed resource.
...
The template code runs the provider with full admin permissions. Before releasing your provider, restrict its required permissions using the [advanced ClusterAccessReconciler](https://github.com/openmcp-project/openmcp-operator/blob/main/lib/clusteraccess/advanced/clusteraccess.go).
...
It is also possible to dynamically generate the `TokenConfig` of your provider which is enabled through registering a [TokenAccessGenerator](https://github.com/openmcp-project/openmcp-operator/blob/045b213389c43c37bf82bf5dda37292b04b7f48d/lib/clusteraccess/advanced/clusterregistration.go#L71) and [AdditionalDataGenerators](https://github.com/openmcp-project/opencontrolplane-runtime/blob/05e1902eb97e06115c7246c6ed42f8a5ca689167/pkg/serviceprovider/apireconciler.go#L52).
...
:::info
Using dynamic cluster access is only possible when using the [advanced ClusterAccessReconciler](https://github.com/openmcp-project/openmcp-operator/blob/main/lib/clusteraccess/advanced/clusteraccess.go).
If your service provider is still using the legacy [simple ClusterAccessReconciler](https://github.com/openmcp-project/openmcp-operator/blob/main/lib/clusteraccess/clusteraccess.go) please refer to the [migration guide](#migrate-from-simple-to-advanced-clusteraccessreconciler) on how to migrate.
:::

To see a real-world example on how to implement dynamic cluster access please check out the implementation of [service-provider-external-secrets](https://github.com/openmcp-project/service-provider-external-secrets/blob/main/cmd/service-provider-external-secrets/main.go).
...
This section walks through migrating a service provider from the legacy [simple ClusterAccessReconciler](https://github.com/openmcp-project/openmcp-operator/blob/main/lib/clusteraccess/clusteraccess.go)
to the [advanced ClusterAccessReconciler](https://github.com/openmcp-project/openmcp-operator/blob/main/lib/clusteraccess/advanced/clusteraccess.go).
...
:::info
To see a real-world implementation have for example a look at [service-provider-flux](https://github.com/openmcp-project/service-provider-flux/blob/main/cmd/service-provider-flux/main.go)
or [service-provider-external-secrets](https://github.com/openmcp-project/service-provider-external-secrets/blob/main/cmd/service-provider-external-secrets/main.go)
:::
...
:::info
Replace the wildcard rules with the minimum set of permissions your provider actually needs.
If you need the permissions to vary per reconcile please refer to [Dynamic Cluster Access](#dynamic-cluster-access).
:::
...
The `template:generate-provider` task provides a `webhook` option if you need to implement any defaulting or validation logic based on webhooks. Note that it is recommended to use [CEL-based admission control](https://kubernetes.io/docs/concepts/cluster-administration/admission-webhooks-good-practices/#choose-admission-mechanism) instead of implementing webhooks when possible.

The challenge of implementing and deploying webhooks in context of OpenControlPlane is that you typically want to realize one of the following two scenarios as a Service Provider:

- A) Deploying a webhook service on the platform cluster that is reachable by the onboarding cluster API server to perform defaulting/validation logic for your service provider API type (see [platform-service-project-workspace](https://github.com/openmcp-project/platform-service-project-workspace) for a real world implementation).
- B) Deploying webhooks that are part of your managed domain service to a workload cluster and realize the routing between tenant control planes and the webhook service (see [service-provider-landscaper](https://github.com/openmcp-project/service-provider-landscaper) for a real world implementation).

Both scenarios can be realized with [platform-service-gateway](https://github.com/openmcp-project/platform-service-gateway) and [platform-service-dns](https://github.com/openmcp-project/platform-service-dns) in OpenControlPlane.

The template provides a fully functional example for scenario A) and uses [openmcp-testing](https://github.com/openmcp-project/openmcp-testing) to realize the routing without a dedicated DNS service in [kind](https://kind.sigs.k8s.io/) managed control planes.
...
will run in either the ControlPlane or the Workload cluster. This depends on your choice, however, it is recommended to deploy into the workload cluster as documented [here](https://openmcp-project.github.io/docs/about/design/service-provider/#non-goals).

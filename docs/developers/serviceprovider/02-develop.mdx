...
Both generators will be called during reconciliation. The AdditionalDataGenerators will generate additional data which is looped through the various ClusterAccessReconciler methods down to the TokenAccessGenerator where it can be used to tailor the TokenConfig to the needs of your service provider.
This if for example useful if your service provider offers a namespace override. The namespace is only known during reconciliation and using a TokenAccessGenerator allows you to take that information into account.

:::info
Using the TokenAccessGenerator is only possible when using the [advanced ClusterAccessReconciler](https://github.com/openmcp-project/openmcp-operator/blob/main/lib/clusteraccess/advanced/clusteraccess.go).
If your service provider is still using the legacy simple ClusterAccessReconciler please refer to TODO!!! on how to migrate.
:::

As an example we will demonstrate how you can dynamically insert an overridden namespace into your TokenConfig. To achieve this we first need to create a named type which will hold the namespace and function which determines it.
...
Now we define a tokenAccessGenerator in the service providers main function. Here we first try to fetch the namespace from the provided additionalData and then use that in the Permissions of our TokenConfig.

```go
tokenAccessGenerator := func(req reconcile.Request, additionalData ...any) (*clustersv1alpha1.TokenConfig, error) {
    namespace := fooService.DefaultNamespace
    for _, data := range additionalData {
        accessScope, ok := data.(*accessScopeData)
        if ok && accessScope != nil && accessScope.namespace != "" {

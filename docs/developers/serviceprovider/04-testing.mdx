...
[`openmcp-testing`](https://github.com/openmcp-project/openmcp-testing) is a testing framework based on [`sigs.k8s.io/e2e-framework`](https://github.com/kubernetes-sigs/e2e-framework). It provisions a kind-based **platform** cluster, installs `openmcp-operator` plus the cluster providers, service providers, and platform services needed to run tests for your service provider. It also provides several helpers to manage the **onboarding** cluster and per-tenant **ControlPlane** clusters to be able to define complex test scenarios.
...
- `test/e2e/mcp/`: resources deployed on the ControlPlane cluster
...
basicTest := features.New("my service").
    ...
    Assess("verify resource reconciles", func(ctx context.Context, t *testing.T, c *envconf.Config) context.Context {
        // assert domain-specific results on the ControlPlane or onboarding cluster
        return ctx
    }).
...
`openmcp-testing` also provides helpers for ControlPlane lifecycle management, cluster access, resource operations, and condition waiting. For the full API, browse the [`pkg/`](https://github.com/openmcp-project/openmcp-testing/tree/main/pkg) directory in the `openmcp-testing` repository.

A typical `Assess` step applies domain objects onto the ControlPlane and waits for them to reconcile:
...
## Testing with multiple ControlPlanes

It's also possible to test and verify tenant isolation or per-tenant configuration. This is achieved by creating multiple ControlPlanes and run the same assertions against each one of them:

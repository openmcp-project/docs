...
:::info
Please adjust the shoot configuration based on your specific needs, e.g. change `Evaluation` to `Production` as purpose, if you are planning to use the managed control plane for productive purposes. For all the details reg. Shoot configuration, please consult the respective Gardener documentation.
:::
